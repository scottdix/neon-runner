...
func _build() -> void:
	...
	add_child(_poc_mode_row(1160.0))


## A 3-segment COMBAT POC selector (#86/#87): LEGACY / KINETIC / GEOM, locked in BEFORE a run (the
## StanceController reads Settings.poc_mode at game_started). Mirrors _difficulty_row: tapping a
## segment commits via Settings.set_poc_mode (persists + announces) and relights the active one.
func _poc_mode_row(y: float) -> Control:
	var row := Control.new()
	row.position = Vector2(90.0, y)
	row.size = Vector2(UI.DESIGN.x - 180.0, 160.0)

	var lab := UI.text("COMBAT (POC)", Fonts.ui, 44, Palette.TEXT_MUTED_HUD)
	lab.position = Vector2(0.0, 0.0)
	row.add_child(lab)

	var seg_labels := ["LEGACY", "KINETIC", "GEOM"]
	var accent := Palette.MENU_MAGENTA_HUD
	var seg_w: float = (row.size.x - 2.0 * 16.0) / 3.0
	var seg_y := 70.0
	var pills: Array[Panel] = []

	var paint := func() -> void:
		var active: int = int(Settings.poc_mode)
		for i in pills.size():
	...
	for i in seg_labels.size():
		var pill := UI.panel(Vector2(seg_w, 84.0), accent, 0.05, 2.0, 14)
		pill.position = Vector2(float(i) * (seg_w + 16.0), seg_y)
		var pl := UI.text(seg_labels[i], Fonts.arcade, 26, UI.TEXT_BRIGHT, HORIZONTAL_ALIGNMENT_CENTER)
